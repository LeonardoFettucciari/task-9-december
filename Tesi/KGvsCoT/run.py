...
torch.set_num_threads(8)  # Use 8 CPU threads for PyTorch
...
for model_index, model_name in enumerate(model_list, 1):
    ...
    for i, sample in iterator:
        ...
        # Generate answers
        raw_answer_zeroshot, answer_zeroshot =                               get_answers(model, tokenizer, prompt_zeroshot, model_name, max_new_tokens=1, device='cuda')
        raw_answer_zeroshot_with_knowledge, answer_zeroshot_with_knowledge = get_answers(model, tokenizer, prompt_zeroshot_with_knowledge, model_name, max_new_tokens=1, device='cuda')
        raw_answer_zeroshot_cot, answer_zeroshot_cot =                       get_answers(model, tokenizer, prompt_zeroshot_cot, model_name, max_new_tokens=512, device='cuda')

        raw_answer_fewshot, answer_fewshot =                                 get_answers(model, tokenizer, prompt_fewshot, model_name, max_new_tokens=1, device='cuda')
        raw_answer_fewshot_with_knowledge, answer_fewshot_with_knowledge =   get_answers(model, tokenizer, prompt_fewshot_with_knowledge, model_name, max_new_tokens=1, device='cuda')
        ...
        sample_output['cot_zeroshot_generated'] = raw_answer_zeroshot_cot   # TODO: Replace with only reasoning after parsing with regex
        sample_output['cot_fewshot_generated'] = raw_answer_fewshot_cot     # TODO: Replace with only reasoning after parsing with regex
